...
2. **Verify workflows run:**
   - Create a test PR to verify `pr.yml` works
...
1. **zqnt-utils access:**
   - Ensure `zqnt-utils` is available in your organization's GitHub Packages
   - Verify GitHub token permissions for private packages

2. **GitHub Packages:**
   - Configure package registry URL if using custom registry
